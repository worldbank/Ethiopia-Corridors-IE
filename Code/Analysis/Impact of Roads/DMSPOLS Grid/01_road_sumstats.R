...
roads_1996 <- readOGR(dsn=".", layer=paste0("All_Network_","1996")) %>% spTransform(CRS(UTM_ETH))
roads_2016 <- readOGR(dsn=".", layer=paste0("All_Network_","2016")) %>% spTransform(CRS(UTM_ETH))

roads_2015 <- readOGR(dsn=".", layer=paste0("All_Network_","2015")) %>% spTransform(CRS(UTM_ETH))

aa <- roads_2015[roads_2015$LINKNAME %in% "Addis - Adama (Toll Road)",]



roads_2016$LINKNAME[grepl("Addis Adama", roads_2016$LINKNAME)] %>% unique

length_1996 <- gLength(roads_1996) / 1000
length_2016 <- gLength(roads_2016) / 1000

roads_2016$LINKLENGTH %>% sum
roads_1996$LINKLENGTH %>% sum
