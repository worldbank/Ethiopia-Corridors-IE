  ...
  for(addis_distance in c("All", "Far")){
    for(unit in c("cell", "woreda")){
      ...
      if(unit %in% "woreda"){
        ...
        lm_region <- felm(dv ~ post_improvedroad*region_type - region_type | cell_id + year | 0 | 0, data=data_temp)
        lm_50aboveafter_region <- felm(dv ~ post_improvedroad_50aboveafter*region_type - region_type | cell_id + year | 0 | 0, data=data_temp)
        lm_below50after_region <- felm(dv ~ post_improvedroad_below50after*region_type - region_type | cell_id + year | 0 | 0, data=data_temp)
      }
