...
for(dv in c("dmspols_zhang_ihs", "dmspols_zhang_6", "globcover_urban", "globcover_cropland")){
  ...
  #        data_w_temp <- data_w
        ...
        if(unit %in% "woreda"){
          ...
          lm_50aboveafter <- felm(dv ~ post_improvedroad_50aboveafter | cell_id + year | 0 | cluster_var, data=data_temp)
          lm_below50after <- felm(dv ~ post_improvedroad_below50after | cell_id + year | 0 | cluster_var, data=data_temp)
          
          # INTERACT BASELINE NIGHTTIME LIGHTS
          lm_baselineNTL <- felm(dv ~ post_improvedroad*dmspols_zhang_1996_group_woreda - dmspols_zhang_1996_group_woreda | cell_id + year | 0 | cluster_var, data=data_temp)
          lm_50aboveafter_baselineNTL <- felm(dv ~ post_improvedroad_50aboveafter*dmspols_zhang_1996_group_woreda - dmspols_zhang_1996_group_woreda | cell_id + year | 0 | cluster_var, data=data_temp)
          lm_below50after_baselineNTL <- felm(dv ~ post_improvedroad_below50after*dmspols_zhang_1996_group_woreda - dmspols_zhang_1996_group_woreda | cell_id + year | 0 | cluster_var, data=data_temp)
          
          # INTERACT REGION GROUP
          lm_region <- felm(dv ~ post_improvedroad*region_type - region_type | cell_id + year | 0 | cluster_var, data=data_temp)
          lm_50aboveafter_region <- felm(dv ~ post_improvedroad_50aboveafter*region_type - region_type | cell_id + year | 0 | cluster_var, data=data_temp)
          lm_below50after_region <- felm(dv ~ post_improvedroad_below50after*region_type - region_type | cell_id + year | 0 | cluster_var, data=data_temp)
        }
