...
if(SCALE_BY_AREA){
  ...
  for(var in road_vars){
    woreda_rdlngth[[var]] <- woreda_rdlngth[[var]] / woreda_rdlngth$area
  }
}
...
# Export -----------------------------------------------------------------------
saveRDS(neighbor_stat_df, file.path(finaldata_file_path, DATASET_TYPE, "individual_datasets", paste0("road_accessibility_scaleArea",SCALE_BY_AREA,".Rds")))
