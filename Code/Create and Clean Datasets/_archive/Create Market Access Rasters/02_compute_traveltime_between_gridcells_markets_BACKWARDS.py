...
def calc_MA_per_cell(cell_id):
    ...
    traveltime_list = np.array(list(traveltime_all_df_i.iloc[0]), dtype=np.float) + list(points_gdp_Gsnap_i.NN_dist)[0] * 2
...
# Loop through year -----------------------------------------------------------
for YEAR in ['2016', '2014', '2012', '2010','2008']:
    ...
    markets_gdp_Gsnap = gn.pandana_snap(G, markets_gdp, add_dist_to_node_col=True)
    points_gdp_Gsnap = gn.pandana_snap(G, points_gdp, add_dist_to_node_col=True)
