...
# removed
points_all <- data.frame(cell_id = 1:15670)
...
for(dataset in DATASETS_TIME_VARYING){
  print(dataset)
  dataset_temp <- readRDS(file.path(panel_rsdp_imp_data_file_path, "kebele", "individual_datasets", dataset)) %>% data.table 
...
# Export -----------------------------------------------------------------------
saveRDS(points_all, file.path(panel_rsdp_imp_data_file_path, "kebele", "merged_datasets", "panel_data.Rds"))
