...
# Load Data --------------------------------------------------------------------
data2012 <- readRDS(file.path(panel_rsdp_imp_data_file_path, "clusters_of_globcover_urban", "merged_datasets",
                              "longdiff_data_clean_base1996_end2012.Rds"))
data2016 <- readRDS(file.path(panel_rsdp_imp_data_file_path, "clusters_of_globcover_urban", "merged_datasets",
  ...
  for(log in c("_log")){
    for(exclude100 in c("")){
      ...
      stargazer(lm_dmspols_zhang_ihs,
                ...
                dep.var.labels   = c("IHS(NTL)", "NTL$>$2", "NTL$>$6", "Clstr. Exists", "Urban", "Crop",  "NDVI"),
                keep=c("MA_var"),
