...
extract_precip_to_points <- function(year, points){
  print(year)
  temp_avg <- raster(file.path(data_file_path, "Temperature", "RawData", "FEWS_NET_FLDAS", paste0("eth_temp_",year,"_avg_FLDAS_10km.tif")))
  temp_min <- raster(file.path(data_file_path, "Temperature", "RawData", "FEWS_NET_FLDAS", paste0("eth_temp_",year,"_min_FLDAS_10km.tif")))
  temp_max <- raster(file.path(data_file_path, "Temperature", "RawData", "FEWS_NET_FLDAS", paste0("eth_temp_",year,"_max_FLDAS_10km.tif")))
...
# Export -----------------------------------------------------------------------
saveRDS(points_all, file.path(panel_rsdp_imp_data_file_path, DATASET_TYPE, "individual_datasets", "temperature.Rds"))
