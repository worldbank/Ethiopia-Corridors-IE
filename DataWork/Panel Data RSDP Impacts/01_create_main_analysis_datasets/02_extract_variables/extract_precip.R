...
extract_precip_to_points <- function(year, points){
  print(year)
  precip <- raster(file.path(data_file_path, "Precipitation", "RawData", "NOAA_PERSIAN", paste0("eth_precip_",year,"_avg_NOAA_PERSIANN-CDR_25km.tif")))
...
# Export -----------------------------------------------------------------------
saveRDS(points_all, file.path(panel_rsdp_imp_data_file_path, DATASET_TYPE, "individual_datasets", "precipitation.Rds"))
