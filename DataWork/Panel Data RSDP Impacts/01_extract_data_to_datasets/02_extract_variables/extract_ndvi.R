...
if(INCLUDE_NDVI_ONLY_IN_CROPLAND){
  cropland_constant <- raster(file.path(data_file_path, "Globcover", "RawData", "1992_2015_data", "ESACCI-LC-L4-LCCS-Map-300m-P1Y-1992_2015-v2.0.7.tif"), band=(1992-1991)) %>% crop(extent(polygons))
  cropland_constant[] <- as.numeric(cropland_constant[] %in% c(10,20,30))
  
  for(year in 1993:2015){
    print(year)
    cropland_yyyy <- raster(file.path(data_file_path, "Globcover", "RawData", "1992_2015_data", "ESACCI-LC-L4-LCCS-Map-300m-P1Y-1992_2015-v2.0.7.tif"), band=(year-1991)) %>% crop(extent(polygons))
    cropland_constant[] <- as.numeric((cropland_constant[] %in% 1) & (cropland_yyyy[] %in% c(10,20,30))) 
  }
  
  for(year in 2016:2018){
    print(year)
    cropland_yyyy <- raster(file.path(data_file_path, "Globcover", "RawData", "2016_2018_data", paste0("C3S-LC-L4-LCCS-Map-300m-P1Y-",year,"-v2.1.1.tif"))) %>% crop(extent(polygons))
    cropland_constant[] <- as.numeric((cropland_constant[] %in% 1) & (cropland_yyyy[] %in% c(10,20,30))) 
  }
...
# Export -----------------------------------------------------------------------
saveRDS(polygons_ndvi, file.path(panel_rsdp_imp_data_file_path, DATASET_TYPE, "individual_datasets", "ndvi.Rds"))
