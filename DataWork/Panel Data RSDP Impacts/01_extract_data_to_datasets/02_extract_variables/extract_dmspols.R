...
extract_dmspols_to_points <- function(year, sdf){
  ...
  if(grepl("grid", DATASET_TYPE)){
    ...
    sdf$dmspols_10 <- dmspols_vx$extract(sp=sdf, fun=function(x){mean(x >= 10, na.rm=T)}) %>% as.numeric
    sdf$dmspols_15 <- dmspols_vx$extract(sp=sdf, fun=function(x){mean(x >= 15, na.rm=T)}) %>% as.numeric
    sdf$dmspols_20 <- dmspols_vx$extract(sp=sdf, fun=function(x){mean(x >= 20, na.rm=T)}) %>% as.numeric
    sdf$dmspols_25 <- dmspols_vx$extract(sp=sdf, fun=function(x){mean(x >= 25, na.rm=T)}) %>% as.numeric
    sdf$dmspols_30 <- dmspols_vx$extract(sp=sdf, fun=function(x){mean(x >= 30, na.rm=T)}) %>% as.numeric
    ...
    sdf$dmspols_sum10        <- dmspols_vx$extract(sp=sdf, fun=function(x){sum(x >= 10, na.rm=T)}) %>% as.numeric
    sdf$dmspols_sum15        <- dmspols_vx$extract(sp=sdf, fun=function(x){sum(x >= 15, na.rm=T)}) %>% as.numeric
    sdf$dmspols_sum20        <- dmspols_vx$extract(sp=sdf, fun=function(x){sum(x >= 20, na.rm=T)}) %>% as.numeric
    sdf$dmspols_sum25        <- dmspols_vx$extract(sp=sdf, fun=function(x){sum(x >= 25, na.rm=T)}) %>% as.numeric
    sdf$dmspols_sum30        <- dmspols_vx$extract(sp=sdf, fun=function(x){sum(x >= 30, na.rm=T)}) %>% as.numeric
