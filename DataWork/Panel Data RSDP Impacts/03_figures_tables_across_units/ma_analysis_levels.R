    ...
    for(log in c("_log")){
      for(exclude in c("_exclude50km")){ # "_exclude20km", "_exclude100km","_exclude100km"
        ...
        #  if(unit == "clusters_of_globcover_urban") data$MA_var_exc <- data[[paste0("MA_gcu2000_tt_theta",theta,exclude,MA_ubanrural, log)]]
          ...
          ## OLS Regressions
          ols1  <- felm(dmspols_zhang_ihs ~ MA_var                                       + temp_avg + precipitation | year + cell_id | 0                     | Z_CODE, data = data)
          ols2  <- felm(dmspols_zhang_ihs ~ MA_var + MA_varXdmspols_1996_bin4_2 + MA_varXdmspols_1996_bin4_3 + MA_varXdmspols_1996_bin4_4 + temp_avg + precipitation | year + cell_id | 0  | Z_CODE, data = data)
          ols3  <- felm(dmspols_zhang_ihs ~ MA_var + MA_varXdistance_city_addisababa + temp_avg + precipitation | year + cell_id | 0                     | Z_CODE, data = data) 
          
          ols4  <- felm(dmspols_zhang_sum2_ihs ~ MA_var                                       + temp_avg + precipitation | year + cell_id | 0                     | Z_CODE, data = data)
          ols5  <- felm(dmspols_zhang_sum2_ihs ~ MA_var + MA_varXdmspols_1996_bin4_2 + MA_varXdmspols_1996_bin4_3 + MA_varXdmspols_1996_bin4_4 + temp_avg + precipitation | year + cell_id | 0  | Z_CODE, data = data)
          ols6  <- felm(dmspols_zhang_sum2_ihs ~ MA_var + MA_varXdistance_city_addisababa + temp_avg + precipitation | year + cell_id | 0                     | Z_CODE, data = data) 
          
          ols7  <- felm(dmspols_zhang_sum6_ihs ~ MA_var                                       + temp_avg + precipitation | year + cell_id | 0                     | Z_CODE, data = data)
          ols8  <- felm(dmspols_zhang_sum6_ihs ~ MA_var + MA_varXdmspols_1996_bin4_2 + MA_varXdmspols_1996_bin4_3 + MA_varXdmspols_1996_bin4_4 + temp_avg + precipitation | year + cell_id | 0  | Z_CODE, data = data)
          ols9 <- felm(dmspols_zhang_sum6_ihs ~ MA_var + MA_varXdistance_city_addisababa + temp_avg + precipitation | year + cell_id | 0                     | Z_CODE, data = data) 
