...
if(F){
  roads_sdf_p123_wgs84 <- roads_sdf_p123 %>%
    spTransform(CRS("+init=epsg:4326"))
  
  roads_sdf_p123_wgs84$color <- sample(x = c("red",
                                         "yellow",
                                         "blue",
                                         "purple",
                                         "green",
                                         "black",
                                         "orange"),
                                 size = nrow(roads_sdf_p123),
                                 replace = T)
  
  leaflet() %>%
    addTiles() %>%
    addPolylines(data = roads_sdf_p123_wgs84,
                 color = roads_sdf_p123_wgs84$color,
                 label = ~LINKNAME)
  
  roads_sdf_p123_wgs84$N_cities <- str_count(roads_sdf_p123_wgs84$LINKNAME, "-") + 1
  
  roads_sdf_p123_wgs84_g3 <- roads_sdf_p123_wgs84[roads_sdf_p123_wgs84$N_cities >= 3,]
  leaflet() %>%
    addTiles() %>%
    addPolylines(data = roads_sdf_p123_wgs84_g3,
                 color = roads_sdf_p123_wgs84_g3$color,
                 label = ~LINKNAME)
...
saveRDS(roads_sdf_p123, file.path(data_file_path, "RSDP Phase I-III Roads and Targetted Locations", "FinalData", "roads_rsdp_i_iii.Rds"))
saveRDS(endpoints, file.path(data_file_path, "RSDP Phase I-III Roads and Targetted Locations", "FinalData", "targetted_locations.Rds"))
saveRDS(endpoints_unique_df, file.path(data_file_path, "RSDP Phase I-III Roads and Targetted Locations", "FinalData", "targetted_locations_unique.Rds"))



# roads_sdf_p123$colors <- sample(x = c("green",
#                                       "red",
#                                       "yellow",
#                                       "orange",
#                                       "black",
...
# leaflet() %>%
#   addTiles() %>%
#   addPolylines(data = roads_sdf_p123[roads_sdf_p123$i %in% 2,],
#                popup = ~LINKNAME,
#                color = roads_sdf_p123$colors)
