...
for(region in c("All", unique(eth$NAME_1))){
  ...
  # Load Data --------------------------------------------------------------------
  ## Points to Connect
  points_sdf <- readRDS(file.path(data_file_path, "RSDP Phase I-IV Roads and Targetted Locations", "FinalData", "targetted_locations_unique.Rds"))
  ...
  saveRDS(minimal_spanning_tree_sdf,
    ...
    saveRDS(minimal_spanning_tree_sdf,
            file.path(data_file_path, "RSDP Phase I-IV Roads and Targetted Locations", "FinalData", 
                      "mst_by_region",
                      paste0("rsdpi_iv_targetted_loc_eucdist_mst_",region,".Rds")))
  }
