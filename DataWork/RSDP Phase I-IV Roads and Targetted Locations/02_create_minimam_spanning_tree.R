...
#for(region in c("Southern Nations, Nationalities and Peoples", "Tigray")){
  print(paste(region, "------------------------------------------------------"))
  
  # Load Data --------------------------------------------------------------------
  ## Points to Connect
  points_sdf <- readRDS(file.path(data_file_path, "RSDP Phase I-IV Roads and Targetted Locations", "FinalData", "targetted_locations.Rds"))
  ...
  eth_buff <- readRDS(file.path(data_file_path, "GADM", "RawData", "gadm36_ETH_0_sp.rds")) %>%
  ...
  if(region == "All"){
    saveRDS(minimal_spanning_tree_sdf,
            file.path(data_file_path, "RSDP Phase I-IV Roads and Targetted Locations", "FinalData", 
                      "rsdpi_iv_targetted_loc_leastcost_mst.Rds"))
  } else{
    saveRDS(minimal_spanning_tree_sdf,
            file.path(data_file_path, "RSDP Phase I-IV Roads and Targetted Locations", "FinalData", 
                      "mst_by_region",
                      paste0("rsdpi_iv_targetted_loc_leastcost_mst_",region,".Rds")))
  }
