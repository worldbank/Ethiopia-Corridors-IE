...
for(year in 1992:2013){
  ntl <- raster(file.path(project_file_path, "Data", "RawData", "DMSP_OLS", "GEE", paste0("eth_dmspols_",year,".tif"))) %>% crop(cities_buff) %>% mask(cities_buff)
  ntl[][ntl[] < 3] <- 0
  ...
  ggsave(ntl_map, filename=file.path(project_file_path, "Figures", "dmspols","mekele", paste0("dmspols_",year,".png")), height=9, width=7)
}